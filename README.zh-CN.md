...
## 48组配对输入、96次系统运行：稳定工作流

配对评测规模为 **16类公开任务 × 3组固定输入 × 2套系统 = 96次系统运行**，组成 **48组同输入对照**。同一份冻结配置必须连续通过3组固定输入，才计为一个稳定工作流。
...
全部19个Harness工作流中，18/19连续通过3组运行输入；Harness侧共57次系统运行，其中54次通过（94.7%）。
